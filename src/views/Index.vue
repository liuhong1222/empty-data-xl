<template>
  <div class="index" id="index">
    ...
    <div class="router-pages">
      <div class="header-bgcolor" :class="{transparent: path.indexOf('question')>-1}"></div>
      <router-view class="routerPage"/>
    </div>
    <div class="main-footer" v-if="path.indexOf('empty')=== -1">
      <ul class="default-width menu-navig">
        <li><h3>帮助中心</h3>
          <p><a @click="goto('/question')">常见问题</a></p>
          <p><a @click="goto('/howrecharge')">怎么充值</a></p></li>
        <li><h3>走进我们</h3>
          <p><a @click="goto('/news')">新闻中心</a></p></li>
        <li><h3>联系我们</h3>
          <p>客服热线：{{siteInfo.hotline}}</p>
          <p>客服qq：{{siteInfo.qq}}</p>
          <p>商务合作：{{siteInfo.businessCode}}</p></li>
      </ul>
      <ul class="copy-right">
