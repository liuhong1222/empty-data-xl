<template>
  <div class="question">
    <div class="form-regin"><h2>我们随时为您服务</h2>
      <div class="center-box">
        <div class="form-input"><input type="text" placeholder="请输入搜索关键词" v-model="keyword"> <!---->
          <div type="button" class="file-butt" @click="matchcontent"><img height="18" src="../assets/index/search.png">浏览
          </div>
        </div>
        <p class="form-remark">热门关键词：空号检测的原理，各个包的具体含义</p></div>
    </div>
    <div data-v-940d8b7a="" class="default-width content-regin">
      <ul data-v-940d8b7a="" class="left-navig">
        ...
        <li data-v-940d8b7a="" v-for="item in fagList" :key="item.id">
          <a data-v-940d8b7a="" class="nuxt-link-exact-active" :class="{'nuxt-link-active': item.id === selectId}"
...
export default {
  ...
  methods: {
    matchcontent () {
      ...
      this.refresh = this.questioncontent.replace(regEx, `<span style="color: #f3bb02">${this.keyword}</span>`)
    },
    async getFagList () {
      var params = {}
