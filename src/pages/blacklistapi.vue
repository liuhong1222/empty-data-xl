    ...
    <div class="emptyapi-wrap">
      <div class="default-width info-regin">
        <div class="info-list">
          <h2>基本信息</h2>
          <div>当前余额（条）</div>
          <h3>{{ personalInfo.lineDirectBalance | thousandbit }}</h3>
          <a-button
            size="small"
          ...
          >
            ...
            <a-form-model-item prop="mobiles">
              ...
              <p style="line-height: 18px; margin-top: 10px">
                注:手机号码，多个号码以英文半角逗号分隔，单次支持1-500个号码
              </p>
            </a-form-model-item>
            <a-form-model-item prop="forbidLevel">
              <a-select
                placeholder="请选择拦截等级"
                v-model="formTest.forbidLevel"
                >
                <a-select-option
                    :key="item.value"
      ...
      <div class="default-width history-regin">
        ...
        <history-view
          ref="historyRef"
          :historyUrl="'blacklistApiTestRecord'"
          :columns="columns"
        >
        </history-view>
      </div>
...
export default {
  data () {
    return {
      ...
      rulesTest: {
        ...
        forbidLevel: [
          { required: true, message: '请选择拦截等级', trigger: 'blur' }
        ]
      },
      testResList: {},
      testLoading: false,
  ...
  methods: {
    ...
    downloadDoc () {
      ...
      // window.location.href = `${downloadUrl}word/RQAPI.zip`
      // http://域名/pdf/号码状态接口文档.pdf
    ...
    testChecking (event) {
      ...
      if (!this.isCertified) {
        ...
        // 判断是否有余额
        if (
          !this.personalInfo.lineDirectBalance ||
          this.personalInfo.lineDirectBalance === '0'
        ) {
          event.preventDefault()
    ...
    testAjax () {
      ...
      testForm.append('mobiles', this.formTest.mobiles)
      testForm.append('productType', this.formTest.forbidLevel)
      server
        .blacklistApiTest(testForm)
        .then((res) => {
          this.testLoading = false
          this.testResList = res.data || {}
          this.$refs.testdialog.visible = true
          this.$refs.historyRef.getPageByMobile()
        })
        .catch(() => {
          this.testLoading = false
...
.banner-view {
  height: 530px;
  min-width: 1000px;
  background: url('../assets/index/blacklistAPI.jpg') no-repeat center;
