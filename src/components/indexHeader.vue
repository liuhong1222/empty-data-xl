      ...
      <ul class="header-navig-list">
        <li>
          <a
            :class="{
              active:
                path.indexOf('empty') > -1 && path.indexOf('emptyapi') === -1
            }"
            @click="goto(getSessionToken() ? '/empty' : '/')"
            >空号检测</a
          >
        </li>
        <li>
          <a
            :class="{
              active:
                path.indexOf('realtime') > -1 &&
                path.indexOf('realtimeapi') === -1
            }"
            @click="goto(getSessionToken() ? '/realtime' : '/')"
            >实时检测</a
          >
        </li>
        <li>
        ...
        <li>
          <a
            :class="{ active: path.indexOf('aboutwe') > -1 }"
            @click="goto('/aboutwe')"
            >关于我们</a
          >
        </li>
      ...
      <div class="header-right">
        ...
        <template v-else>
          <a-dropdown :trigger="['click']" placement="bottomRight">
            ...
            <a-menu slot="overlay">
              <a-menu-item @click="showCertification"> 认证资料 </a-menu-item>
              <a-menu-item @click="goto('/account')"> 个人账户 </a-menu-item>
              <a-menu-item @click="goto('/testrecord')"> 历史检测 </a-menu-item>
              <a-menu-item @click="goto('/message')"> 我的消息 </a-menu-item>
              <a-menu-item @click="goto('/phonematch')"> 号码匹配 </a-menu-item>
              <a-menu-item @click="logout"> 退出登录 </a-menu-item>
            </a-menu>
          </a-dropdown>
          <button
            type="button"
            class="el-button button-recharge el-button--danger el-button--small"
            @click="goto('/recharge')"
          >
            <span>充值</span>
          </button>
          <span
            class="c-text c-text-danger c-gap-icon-left-small opr-toplist1-new"
            >优惠中</span
          >
        </template>
      </div>
  ...
  methods: {
    showlogin () {
      if (!this.getSessionToken()) {
        ...
        //   window.location.href = this.downloadDomain + '/resource/生蚝号码魔方.rar'
        // } else if (this.siteInfo.domain === 'yfeifei.com') {
        //   window.location.href = this.downloadDomain + '/resource/肥肥号码魔方.rar'
        // } else if (this.siteInfo.domain === 'jinghaoshuju.com') {
        //   window.location.href = this.downloadDomain + '/resource/精号号码魔方.rar'
        // } else if (this.siteInfo.domain === 'shihaoy.com') {
        //   window.location.href = this.downloadDomain + '/resource/实号号码魔方.rar'
        // } else if (this.siteInfo.domain === 'wckhjc.com') {
        //   window.location.href = this.downloadDomain + '/resource/微尘号码魔方.rar'
        // } else if (this.siteInfo.domain === '51ckh.com') {
        //   window.location.href = this.downloadDomain + '/resource/云信通号码魔方.rar'
        // } else if (this.siteInfo.domain === 'buzheny.com') {
        //   window.location.href = this.downloadDomain + '/resource/步正号码魔方.rar'
        // } else if (this.siteInfo.domain === 'haomajc.com') {
        //   window.location.href = this.downloadDomain + '/resource/众之号码魔方.rar'
        // } else if (this.siteInfo.domain === 'khkjc.com') {
        //   window.location.href = this.downloadDomain + '/resource/讯龙号码魔方.rar'
        // } else if (this.siteInfo.domain === 'sp10010.com' || this.siteInfo.domain === '39kh.com') {
        //   window.location.href = this.downloadDomain + '/resource/龙之杰号码魔方.rar'
        // } else if (this.siteInfo.domain === 'www.dameipr.com' || this.siteInfo.domain === 'dameipr.com') {
        //   window.location.href = this.downloadDomain + '/resource/龙之杰号码魔方.rar'
        // } else {
        //   window.location.href = this.downloadDomain + '/resource/号码魔方.rar'
        // }
      }
    },
    // 号码魔方下载
    async getMobileCubePath () {
      if (!this.getSessionToken()) {
        this.$root.$emit('showlogin', true)
      } else {
        var { data } = await server.getMobileCubePath()
        if (data) {
